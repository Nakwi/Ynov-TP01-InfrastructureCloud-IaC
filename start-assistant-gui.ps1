...
$Gui = Join-Path $Root "terraform_gui.py"

if (-not (Test-Path $Gui)) {
    throw "terraform_gui.py introuvable dans $Root"
}
...
if ($python.Count -eq 0) {
    ...
    if ($answer -and -not $answer.ToLower().StartsWith("y") -and -not $answer.ToLower().StartsWith("o")) {
        throw "Python 3 est requis pour lancer l'interface graphique."
    }
    ...
    if ($python.Count -eq 0) {
        throw "Python semble installe mais pas visible dans ce terminal. Ferme/reouvre PowerShell puis relance .\start-assistant-gui.ps1"
    }
}
